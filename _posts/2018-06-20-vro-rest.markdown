...
That method returns void,  so we then use a second method using filter by name to get the object,  so we use this to obtain object ID which is needed by other functions.

    NSXEdgeTrinityBasicController.getAllEdgeSummariesV4

When creating NSX Distributed Logical Router we use same method to create. However when we try and use the second method to get the object it fails. It looks like the DLR object contains an additional property LogicalRouterScope, which it seems like NSX plugin cannot handle and throws an error.

     java.lang.NoSuchMethodException:com.vmware.vshield.edge.dto.trinity.LogicalRouterScope.<init>()

After trying various other NSX plugin methods, NSX offers all its capabilities via [REST API](https://docs.vmware.com/en/VMware-NSX-for-vSphere/6.4/nsx_64_api.pdf). vRealize Orchestrator comes with a HTTP-REST plugin, we can add NSX Managers as vRO managed servers programatically using [RESTHostManager](http://www.vroapi.com/Class/REST/2.2.2/RESTHostManager) or as this is a one off task you can do via the built in workflow.

    HTTP-REST -> Configuration -> Add a REST host
